...
impl MetadataStore {
    ...
    pub fn query_object_field(&self, field_id: ObjectId, object: &Object) -> Option<&FieldDesc> {
        ...
        if let Some(exclusive) = &desc.exclusive {
            let held_by_aliased = object.aliased_id().map(|id| exclusive.contains(&id)).unwrap_or(false);
            let held_by_parent = object.parent_id().map(|id| exclusive.contains(&id)).unwrap_or(false);
            if !exclusive.contains(&object.id()) && !held_by_aliased && !held_by_parent {
                return None;
            }
        }
    ...
    pub fn query_all_object_fields(&self, object: &Object) -> impl Iterator<Item = &FieldDesc> {
        let object_kind = object.kind();
        let object_id = if let Some(id) = object.aliased_id() {
            id
        } else if let Some(id) = object.parent_id() {
            id
        } else {
            object.id()
        };
...
pub fn read_metadata_dir<P: AsRef<Path>>(path: P) -> MetadataStore {
    ...
    metadata.add_field(
        FieldDesc {
            ...
            value_ty:     ValueType::Real,
            value_ty_raw: "real".to_string(),
            exclusive:    None,
            kind:         ObjectKind::LIGHTNING,
            is_profile:   false,
        }

...
return function(data)
    ...
    assert(version == 11 or version == 12, "Unsupported W3E version: " .. version)
    -- W3E 12 expands the texture/flags field from one byte to two.
    -- Heights and cliff layers retain their encoding, but vertices are 8 bytes.
    local vertexFormat = version == 12 and "<I2I2I2BB" or "<I2I2BBB"
    local vertexSize = string.packsize(vertexFormat)
    ...
    assert(width * height <= (#data - pos + 1) // vertexSize, "Truncated W3E vertices")
    local heights = {}
    for y = 1, height do
        local row = {}
        for x = 1, width do
            local ground, _, _, _, cliff
            ground, _, _, _, cliff, pos = string.unpack(vertexFormat, data, pos)
